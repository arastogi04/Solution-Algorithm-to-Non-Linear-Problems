...
for n = F1ext   %Load step loop
    ...
    while i<itr_max %Newton iterations loop
        ...
        d = d+del_d; %Update the solution vector for the iteration i+1 (stop if solution converged)
...
xlabel('Load Steps');
ylabel('Iterations');
title("Newton-Raphson for x = "+x);

plot(d_value,F1ext');
xlabel('d1');
ylabel('N1(d1)');
title("Numerical Solution (Newton-Raphson) for x = "+x);
